...
public class MyMessageActivity extends BaseActivity {
            ...
            switch (msg.what) {
                case Constants.SUCCESS:
                    ...
                    if (htype == 0) {//进来初始化 和刷新
                        ...
                        //下拉刷新结束
                        alv_my_message.onRefreshComplete();
                        if (messageContentList.size() <= 0) {
                            alv_my_message.setResultSize(0);
                        }

                        alv_my_message.setResultSize(messageContentList.size());

                    } else if (htype == 2) {//加载更多
                        alv_my_message.onLoadComplete();
                        messageContentList.addAll(newList);
                        alv_my_message.setResultSize(newList.size());
                    } else {//条件查询
                        alv_my_message.onRefreshComplete();
                        messageContentList.clear();
                        messageContentList.addAll(newList);
                ...
                case Constants.ERROR:
                    ...
                    if (msgStr == null || msgStr.equals("")) {
                        ToastUtil.showNetworkError(MyMessageActivity.this);
                    } else {
                        ToastUtil.show(MyMessageActivity.this, msgStr);
                    }
    ...
    @BindView(R.id.et_my_message_keyword)
    EditText et_my_message_keyword;
    @BindView(R.id.alv_my_message)
    AutoListView alv_my_message;
    ...
    private void initViews() {
        ...
        messageListViewAdpater = new MessageListViewAdpater(MyMessageActivity.this, messageContentList);
        alv_my_message.setAdapter(messageListViewAdpater);
        messageListViewAdpater.setType(3);
        alv_my_message.setOnRefreshListener(new AutoListView.OnRefreshListener() {
            @Override
        ...
        alv_my_message.setOnLoadListener(new AutoListView.OnLoadListener() {
            @Override
            public void onLoad() {
                page++;
                httpType = 2;
        ...
        et_my_message_keyword.addTextChangedListener(new TextWatcher() {
            @Override
        ...
        messageListViewAdpater.setOnClickListener(new MessageListViewAdpater.OnClickListener() {
            ...
            public void onUpdateMessage(MessageContent messageContent) {
                Intent intent = new Intent(MyMessageActivity.this, AddLostFoundActivity.class);
    ...
    private void showComment(final int messageId, final int messagePosition) {
        commentDialog = new Dialog(MyMessageActivity.this, R.style.mystyle);// 创建自定义样式dialog
        ...
        ll_message_commnet_save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (et_message_comment.getText().toString().equals("")) {
                    ToastUtil.show(MyMessageActivity.this, "请输入信息");
                    return;
                }
    ...
    private void showReply(final User commentUser, final int messageId, final int messagePosition) {
        ...
        commentDialog = new Dialog(MyMessageActivity.this, R.style.loading_dialog);// 创建自定义样式dialog
        commentDialog.setContentView(R.layout.message_comment_item);// 设置布局
        final EditText et_message_comment = (EditText) commentDialog.findViewById(R.id.et_message_comment);
    ...
    private void DeleteCommentById(int commentId, final int messagePosition, final int commentPosition) {
        NetCallBackVolley ncbv = new NetCallBackVolley() {
            ...
            public void onMyRespone(String response) {
                ...
                try {
                    ...
                    if (status) {
                        ToastUtil.show(MyMessageActivity.this, "删除成功");
        ...
        if (Utils.isNetworkAvailable(MyMessageActivity.this)) {
            dialog = Utils.createLoadingDialog(MyMessageActivity.this, getString(R.string.app_dialog_delete));
            dialog.show();
    ...
    private void saveData(Comment comment, final int messagePosition) {
        ...
        if (Utils.isNetworkAvailable(MyMessageActivity.this)) {
            dialog = Utils.createLoadingDialog(MyMessageActivity.this, getString(R.string.app_dialog_save));
            dialog.show();
    ...
    private void GetMessageByUserId() {
        ...
        NetCallBackVolley ncbv = new NetCallBackVolley() {
            @Override
            public void onMyRespone(String response) {
                MLog.i("获取我发布的信息:" + response);
                try {
                    JSONObject jsonObject = new JSONObject(response);
                    int responseCode = jsonObject.getInt("code");
                    if (responseCode == ResponseCode.QUERY_SUCCESS) {
                        JSONObject jsonData = jsonObject.getJSONObject("data");
                        String result = jsonData.getString("blogList");
                        Message msg = new Message();
                        msg.obj = result;
        ...
        if (Utils.isNetworkAvailable(MyMessageActivity.this)) {
            String path = Urls.GetMessageByUserId;
            path = path + "?userid=" + user.getId();
            MLog.i("获取我发布的信息：" + path);
            HttpVolley.volleStringRequestGetString(path, ncbv, handler);
        }
    }
